import{u as y,a as k,r as i,S,J as x,j as e,B as m,A as w,P as D,Q as A}from"./index-C6a7POC2.js";const L=()=>{const{scheduleId:d}=y(),c=k(),l=i.useRef(null),[s,b]=i.useState(null),[n,p]=i.useState([]),[j,o]=i.useState(!0);i.useEffect(()=>{d&&u()},[d]);const u=async()=>{try{o(!0);const r=await S.getScheduleWithJobs(d);r.success&&r.schedule&&r.jobs?(b(r.schedule),p(r.jobs)):(x.error(r.error||"Failed to load schedule"),c("/schedules"))}catch(r){console.error("Error loading schedule:",r),x.error("Failed to load schedule"),c("/schedules")}finally{o(!1)}},g=()=>{c("/schedules")},N=()=>{l.current&&(l.current.style.display="block",l.current.style.visibility="visible",l.current.querySelectorAll("svg, canvas, .qrcode").forEach(a=>{a.style.display="block",a.style.visibility="visible"})),setTimeout(()=>{window.print()},500)},h=r=>new Date(r).toLocaleDateString(),v=()=>s?`${window.location.origin}/schedules/display/${s.id}?schedule=${s.schedule_number}&date=${s.schedule_date}&vehicle=${s.vehicle}&verified=true`:"";return j?e.jsx("div",{className:"flex items-center justify-center h-64",children:e.jsx("div",{className:"text-lg",children:"Loading schedule..."})}):s?e.jsxs("div",{className:"min-h-screen bg-white",children:[e.jsx("style",{children:`
          @media print {
