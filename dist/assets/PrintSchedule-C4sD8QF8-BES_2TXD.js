import{T as y,k,N as i,l as w,i as x,n as e,U as m,u as S,o as D,X as T}from"./index-V59AiG1o.js";const L=()=>{const{scheduleId:d}=y(),a=k(),l=i.useRef(null),[s,b]=i.useState(null),[c,p]=i.useState([]),[j,n]=i.useState(!0);i.useEffect(()=>{d&&u()},[d]);const u=async()=>{try{n(!0);const r=await w.getScheduleWithJobs(d);r.success&&r.schedule&&r.jobs?(b(r.schedule),p(r.jobs)):(x.error(r.error||"Failed to load schedule"),a("/schedules"))}catch(r){console.error("Error loading schedule:",r),x.error("Failed to load schedule"),a("/schedules")}finally{n(!1)}},N=()=>{a("/schedules")},g=()=>{l.current&&(l.current.style.display="block",l.current.style.visibility="visible",l.current.querySelectorAll("svg, canvas, .qrcode").forEach(r=>{r.style.display="block",r.style.visibility="visible"})),setTimeout(()=>{window.print()},500)},o=r=>new Date(r).toLocaleDateString(),v=()=>s?`${window.location.origin}/schedules/display/${s.id}?schedule=${s.schedule_number}&date=${s.schedule_date}&vehicle=${s.vehicle}&verified=true`:"";return j?e.jsx("div",{className:"flex items-center justify-center h-64",children:e.jsx("div",{className:"text-lg",children:"Loading schedule..."})}):s?e.jsxs("div",{className:"min-h-screen bg-white",children:[e.jsx("style",{children:`
          @media print {
