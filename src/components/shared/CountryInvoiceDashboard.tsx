...
export interface CountryInvoice {
  ...
  jobNumber?: string;
  pricing?: { net?: number; gross?: number };
  [key: string]: any;
}

interface CountryInvoiceDashboardProps {
  countryName: string;
  countrySlug: string; // e.g. "saudi-arabia", "philippines"
  storageKey: string; // localStorage key e.g. "saudiArabiaInvoices"
  manifestStorageKey?: string;
  layoutTitle?: string;
}

const CountryInvoiceDashboard: React.FC<CountryInvoiceDashboardProps> = ({
  countryName,
  countrySlug,
  storageKey,
  manifestStorageKey,
  ...
  const [invoices, setInvoices] = useState<CountryInvoice[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedInvoices, setSelectedInvoices] = useState<string[]>([]);
  const [showManifestOptions, setShowManifestOptions] = useState(false);

  const manifestKey = manifestStorageKey || `${countrySlug}Manifests`;
  ...
  const filteredInvoices = invoices.filter(invoice =>
    ...
    String(invoice.jobNumber || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleNewInvoice = () => navigate(`/${countrySlug}/invoice/add`);
  ...
  const handleShareWhatsApp = (invoice: CountryInvoice) => {
    ...
    const message = encodeURIComponent(
      ...
      `💰 Total: QAR ${parseFloat(String(invoice.pricing?.net || invoice.total || '0')).toFixed(2)}\n` +
  ...
  const handleCreateManifest = (type: 'air' | 'sea') => {
    ...
    const manifests = JSON.parse(localStorage.getItem(manifestKey) || '[]');
    ...
    navigate(`/${countrySlug}/manifest/${type}/${manifestId}`);
  ...
  return (
    <Layout title={layoutTitle || `${countryName} Invoices`}>
      <div className="container mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-foreground">{countryName} Invoices</h1>
            <p className="text-muted-foreground mt-1">Manage your {countryName} shipping invoices</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button onClick={() => navigate(`/${countrySlug}/schedules`)} variant="outline" className="flex items-center gap-2">
              📅 Schedules
            </Button>
        ...
        <div className="mb-6">
          <input
            type="text"
            placeholder="Search by invoice number, shipper, consignee, or job number..."
                  ...
                  <tbody>
                    {filteredInvoices.map((invoice) => (
                      <tr key={invoice.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="py-3 px-2 font-medium">QAR {parseFloat(String(invoice.pricing?.net || invoice.total || '0')).toFixed(2)}</td>
                        <td className="py-3 px-2 text-center">
                          <Button variant="outline" size="sm" onClick={() => handleShareWhatsApp(invoice)} title="Share via WhatsApp" className="text-green-600 hover:bg-green-50">
                        ...
                        <td className="py-3 px-2">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <Button variant="outline" size="sm" onClick={() => handlePrintInvoice(invoice.id)} title="Preview/Print">
...
export default CountryInvoiceDashboard;
