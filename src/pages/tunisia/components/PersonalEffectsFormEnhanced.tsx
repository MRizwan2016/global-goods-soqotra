...
const PersonalEffectsFormEnhanced: React.FC<PersonalEffectsFormEnhancedProps> = ({
  ...
  // Calculate volume from dimensions (convert inches to CBM)
  const calculateVolume = (length: number, width: number, height: number): number => {
    // Convert inches to meters and calculate CBM
    const lengthM = length * 0.0254;
    const widthM = width * 0.0254;
    const heightM = height * 0.0254;
    return Number((lengthM * widthM * heightM).toFixed(3));
  };

  // Auto-calculate weight from volume (1 CBM = 1000 KG)
