...
const KenyaInvoiceForm = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        console.error('Error loading Kenya books:', err);
      }
    };
    fetchBooks();
  }, []);
  ...
  const handleGoBack = () => navigate("/kenya-ops");
  const handleSave = async () => {
    const success = await saveInvoice();
    if (success) navigate("/kenya-ops");
  };
  ...
  return (
    <Layout title={`${id ? 'Edit' : 'Add New'} Invoice - Kenya`}>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-3xl font-bold text-[#1e2a3a]">{id ? 'Edit' : 'Add New'} Invoice - Kenya</h1>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.port} onValueChange={(v) => handleFormChange('port', v)}>
                  ...
                  <SelectContent>
                    {kenyaPorts.map(p => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={formData.sector} onValueChange={(v) => handleFormChange('sector', v)}>
                  ...
                  <SelectContent>
                    {kenyaSectors.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={formData.salesRep} onValueChange={(v) => handleFormChange('salesRep', v)}>
                  ...
                  <SelectContent>
                    {kenyaSalesReps.map(r => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={formData.driver} onValueChange={(v) => handleFormChange('driver', v)}>
                  ...
                  <SelectContent>
                    {kenyaDrivers.map(d => <SelectItem key={d.value} value={d.value}>{d.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={formData.district} onValueChange={(v) => handleFormChange('district', v)}>
                  ...
                  <SelectContent>
                    {kenyaDistricts.map(d => <SelectItem key={d.value} value={d.value}>{d.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
              ...
              {formData.doorToDoor === "YES" && (
                <div className="space-y-2">
                  ...
                  <Input value={getDoorToDoorPricing()} readOnly className="bg-green-50" />
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.shipperCity} onValueChange={(v) => handleFormChange('shipperCity', v)}>
                  ...
                  <SelectContent>{(formData.shipperCountry === 'QATAR' ? qatarCities : kenyaCities).map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">MOBILE:</label>
                <Input value={formData.consigneeMobile} onChange={(e) => handleFormChange('consigneeMobile', e.target.value)} placeholder="Mobile (+254...)" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">EMAIL:</label>
            ...
            {packageItems.length > 0 && (
              <div className="grid grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-700">{formData.totalPackages}</div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-500">{formData.totalVolume.toFixed(3)} m³</div>
        ...
        {/* Costs */}
        <Card>
          <CardHeader className="bg-green-700 text-white"><CardTitle>COSTS & PRICING</CardTitle></CardHeader>
          <CardContent className="space-y-6 pt-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-700">{formData.gross.toFixed(2)} {CURRENCY}</div>
          ...
          <CardContent className="pt-6">
            <div className="flex justify-between">
              ...
              <div className="flex gap-2">
                <Button onClick={handleSave} className="gap-2 bg-green-700 hover:bg-green-800"><Save className="h-4 w-4" />{id ? 'Update' : 'Save'} Invoice</Button>
              </div>
            </div>
          </CardContent>
        </Card>
...
export default KenyaInvoiceForm;
