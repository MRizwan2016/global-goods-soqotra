...
import { doorToDoorPricing, sudanPackageTypes, calculateVolumeWeight, calculateCubicFeet, countryCodes } from "../data/sudanOpsData";
...
export interface SudanOpsFormData {
...
export const useSudanOpsInvoice = (invoiceId?: string) => {
  ...
  const [packageItems, setPackageItems] = useState<SudanOpsPackageItem[]>([]);
  ...
  useEffect(() => {
    if (formData.doorToDoor === "YES" && formData.sector && formData.totalWeight > 0) {
      ...
      if (pricing) {
        const amount = formData.totalWeight * pricing.price;
        setFormData(prev => ({ ...prev, doorToDoorPrice: amount, destinationDoorDelivery: amount }));
      }
    } else {
  ...
  const handleFormChange = (field: keyof SudanOpsFormData, value: any) => {
    ...
    setPackageInput(prev => {
      ...
      if (['length', 'width', 'height'].includes(field)) {
        ...
        if (l > 0 && w > 0 && h > 0) {
          ...
          updated.weight = (cbm * 1000).toFixed(2);
        }
      }
      return updated;
    });
  ...
  const addPackageItem = async () => {
    ...
    const newItem: SudanOpsPackageItem = {
  ...
  const saveInvoice = async () => {
    try {
      ...
      if (savedId) {
        ...
        toast.success("Sudan invoice saved successfully!");
        return true;
      } else {
      ...
      console.error("❌ SUDAN INVOICE SAVE ERROR:", error);
      toast.error("Failed to save Sudan invoice");
      return false;
    }
  };
    ...
    try {
      ...
      if (result) {
        ...
        setFormData(prev => ({
          ...
          consigneeCountry: inv.consignee_country || 'SUDAN',
