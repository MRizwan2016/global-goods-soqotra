...
import { ArrowLeft, Plus, Trash2, Upload, Car, Package } from "lucide-react";
import { AlgeriaInvoice, AlgeriaCustomer } from "../types/algeriaInvoiceTypes";
import { VEHICLE_RATES, PERSONAL_EFFECTS_RATE } from "../types/algeriaTypes";
import { AlgeriaStorageService } from "../services/AlgeriaStorageService";
import { AlgeriaInvoiceBookService, AlgeriaInvoiceBook } from "../services/AlgeriaInvoiceBookService";
import PersonalEffectsFormEnhanced from "./PersonalEffectsFormEnhanced";
import { toast } from "sonner";
import { v4 as uuidv4 } from 'uuid';

interface AlgeriaInvoiceFormProps {
  onBack: () => void;
  onInvoiceSave: (invoice: AlgeriaInvoice) => void;
  existingInvoice?: AlgeriaInvoice;
}

const AlgeriaInvoiceForm: React.FC<AlgeriaInvoiceFormProps> = ({
  onBack,
  onInvoiceSave,
  existingInvoice
}) => {
  const [customer, setCustomer] = useState<AlgeriaCustomer>(
  ...
  const [hasVehicle, setHasVehicle] = useState(
    existingInvoice?.vehicle ? true : true
  );
  ...
  const loadAvailableBooks = () => {
    const books = AlgeriaInvoiceBookService.getAvailableBooks();
  ...
  const handleSave = async () => {
    try {
      const existingInvoices = await AlgeriaStorageService.loadInvoices();
      ...
      // Check for duplicate export plate
      if (hasVehicle && vehicle.exportPlate?.trim()) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">
          {existingInvoice ? 'Edit' : 'Create'} Algeria Invoice
        </h1>
      </div>
      ...
      {hasVehicle && (
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="border-2 border-amber-400 bg-amber-50 p-2 rounded">
              ...
              <Input
                ...
                onChange={(e) => setVehicle(prev => ({ ...prev, exportPlate: e.target.value.toUpperCase() }))}
                placeholder="ENTER EXPORT PLATE"
                className="uppercase bg-white border-amber-300 mt-1"
              />
              <p className="text-xs text-amber-700 mt-1">Used for container loading identification</p>
            </div>
            <div>
                ...
                {personalEffects.map((effect, index) => (
                  <div key={index} className="border rounded-lg p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-sm">
                      ...
                      <div>
                        <span className="font-medium">Weight:</span> {effect.weight} KG
                      </div>
                      <div>
      ...
      <div className="flex gap-4">
        ...
        <Button 
          onClick={handleSave} 
          className="flex-1"
          disabled={!customer.name || (hasVehicle && (!vehicle.make || !vehicle.chassisNumber || !vehicle.exportPlate)) || (!hasVehicle && personalEffects.length === 0)}
        >
...
export default AlgeriaInvoiceForm;
